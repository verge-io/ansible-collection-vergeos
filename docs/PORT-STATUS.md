...
"Add site_info for read-only site preflight (#148)".
...
There is no collection-wide `all`, `info`, or `management` group. The
2026-09-11 draft described those three groups growing when the 18 modules
were registered. That registration is not in this file. The runtime
comment says a collection-wide `all` group was left for the rest of the
port.
...
draft could not show the re-enable, because D1 swallowed the write. The
draft's conclusion was that the behavioural half is a prerequisite for
fixing D1.
...
Raising the floor is a supported-version statement.
...
defined, while each module binds its own reference at import. Pre-importing
the module made `test_vm_clone` open real TCP connections. Fixed by
patching the module under test, and by dropping the `sys.modules` stub of
pyvergeos. Issue #66. `tests/unit/test_fixture_discipline.py` refuses that
stub.
...
- `site_sync`, `site_sync_info`, and a `dr_replication` role. Issue #38
  ...
  call those names.
- A collection-wide action group. Only `recipe` is declared.
- The numbered register B1–B7, B9–B20 as one document.
