...
**1.6.1**. See `docs/SDK-COMPATIBILITY.md`. This page was not re-diffed
...
`ClusterStatus.can_lose_one_node()` is not a drop-in for the drain check:
it divides `online_ram` evenly across `online_nodes`, so on uneven nodes a
true result is not an N-1 guarantee (pyvergeos 1.6.1, #135). The drain
check uses each node's `vm_ram`.
...
Not referenced by name anywhere under `plugins/`. Not re-checked against
1.6.1.
...
`rows=0` on that system — command endpoints. System-snapshot restore,
file-level recovery from a snapshot, and making remote snapshots immutable
run through these. None of them is called by this collection.
...
## Not gaps in the 1.2.3 SDK — worth keeping
...
- **`nodes/{key}/maintenance_reboot`.** `nodes.restart()` POSTs here. The
  ...
  than hand-rolling drain, wait, and restart.
...
These were the highest-value items on the 2026-09-09 page: the endpoint
...
`'/users/' in 'users/1'` is false, so a platform-created membership,
...
re-test `GroupMember.member_type` on 1.6.1.
...
contains both. Name lookup in this collection stays client-side because
...
### Non-2xx bodies (the old B9 note)
...
2. `create()` discards the POST body, which is where `response.vm` reports
   the new VM's key.
3. Simulate replies on a non-2xx status, so `_request` throws the document
   away.
...
| `machine_drive_stats` | not modelled | modelled; collection still filters `parent_drive`, because path-key access addresses `$key` |
| P1 tables (~12) | unreachable | not called by this collection; not re-diffed |
| P2 tables | unreachable | not called by this collection; not re-diffed |
| OData `'` and `{` | `'` broken; `{` broken until 1.2.8 | fixed on the floor; lookups stay client-side for ambiguous names (#72) |
| Member `member_type` / `member_key` | wrong on bare `users/N` | collection parses the ref itself (#92); SDK property not re-tested here |
| Simulate body on HTTP 405 | discarded by `_request` | kept by `raw_request()` |

The 97-name list is `docs/pyvergeos-missing-signal.txt`.
