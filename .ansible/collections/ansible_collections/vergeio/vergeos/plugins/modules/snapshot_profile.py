  ...
  periods:
    ...
    suboptions:
      ...
      month:
        ...
        default: 0
      skip_missed:
        description: Skip a snapshot whose scheduled time was missed.
        type: bool
        default: false
      max_tier:
...
def get_profile(client, name):
    try:
        return client.snapshot_profiles.get(name=name)
...
def reconcile_periods(module, client, profile_key, actions):
    ...
    existing = {dict(p).get('name'): p for p in manager.list()}
...
def main():
    ...
    argument_spec.update(
        ...
        periods=dict(
            ...
            options=dict(
                ...
                skip_missed=dict(type='bool', default=False),
    ...
    try:
        profile = get_profile(client, name)
