...
- **Unattended install and new-system discovery** (#56, #58). `docs/UNATTENDED-INSTALL.md` and `examples/unattended_install/` seed a silent install (new system, controller join, scale-out), deploy it as a nested lab, find the new system by the `/api/v4/version` login challenge, and apply a tag baseline. Core-fabric create stays on the API because `network` has no `ipaddress_type`. The installer CD stays on the API because `drive` has no `media_source`. Nested-virt flags the `vm` module does not expose are set the same way. Fabric NICs use `nic`. Tag categories set `taggable_vms` because an omitted flag is stored false on create (#121). pyVergeOS #97 (`save()` skipping alias translation) is fixed on the 1.6.1 floor; this baseline only creates tags.
...
- **`auth_source_info`: a named lookup used `get(name=)` and could return the wrong source** (#72). The SDK single-get stops at the first row. The mutating `auth_source` module already resolves names with `resolve_one` and refuses when more than one row matches. The info module now uses that same client-side match. An unknown name still returns an empty list. Settings are still read by key, and `client_secret` is still stripped.

- **`vm_import`: `state=absent` could not remove a name that had been imported twice** (#162). VergeOS keeps a `vm_imports` row after the VM is deleted and does not require import names to be unique. Absent looked the row up with `resolve_one`, which refuses when two rows share a name, so the cleanup task failed and left both rows. It now removes every finished record of that name, and does not stop at the first match. A record that is still importing blocks that sweep. `import_key` removes one record. The image-pipeline role deletes the record it just created, the image-pipeline ladder removes rows named for its template, and `assert_lab_clean.yml` reads `vm_imports`.

- **`vm_snapshot`: restore and delete could act on a different VM when `snapshot_id` belonged to it** (#161). Naming VM A and passing B's snapshot reverted B (or, on delete, removed B's snapshot) and reported A's id. Before check mode and before the write, the snapshot's `machine` is compared to the named VM's `machine`. A mismatch is refused and the message names both VMs. pyVergeOS #174 makes a snapshot read scoped to the VM raise not found for another machine's key, the same exception a missing key raises. On that error only, the module reads the snapshot once with no VM scope. A row that exists on another machine is still refused and the message names both VMs. A key that does not exist still says not found. Delete that passes only `snapshot_id` is unchanged.
