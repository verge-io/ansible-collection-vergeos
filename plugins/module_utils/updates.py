...
def apply_installed(settings, force=False):
    ...
    ``update_all`` is the documented route. pyvergeos describes it as:
    download available packages, install them, then "reboot nodes one at a
    time with workload migration". Once updates are already installed the
    first two steps have nothing to do, and it does NOT re-check the source --
    ``check`` maps to the ``refresh`` action and ``all`` is its own -- so this
    applies the update that was reviewed and installed, not a newer one.

    An earlier version of this function preferred a narrower ``apply`` action
    and fell back to ``update_all``::
    ...
    MANAGER, not on the settings model that ``get()`` returns. So the
    ``getattr`` always found None, the "fallback" was the only path ever
    taken, and the branch above it was unreachable code that read like the
    main one. That is exactly the defect the RAW_FIELD comment at the top of
    this file records -- a fallback that looks like a safety net and can never
    fire -- made twice in one module.

    The narrower action is reachable as ``settings._manager._action('apply')``
    and is not used here, because whether the API accepts ``apply`` for
    update_settings has not been tested: the only way to find out is to reboot
    a cluster. ``update_all`` is tested by every upgrade anyone has run.
