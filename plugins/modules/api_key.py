...
def join_ip_list(value):
    """Module list param -> raw API string (comma-joined)."""
    if not value:
        return ''
    return ','.join(value)
...
def split_ip_list(value):
    """Raw API string -> list for return values."""
    if not value:
        return []
...
def find_keys(client, user, name):
    """All keys matching (user, name); raw rows carry user_name."""
    matches = []
...
def create_key(module, client, actions):
    ...
    if params.get('ip_allow_list'):
        create_args['ip_allow_list'] = params['ip_allow_list']
    if params.get('ip_deny_list'):
        create_args['ip_deny_list'] = params['ip_deny_list']

    created = client.api_keys.create(**create_args)
    key_obj = client.api_keys.get(created.key)
...
def update_key(module, client, key_obj, actions):
    ...
    if params.get('description') is not None:
        if (current.get('description') or '') != params['description']:
            changes['description'] = params['description']

    for param in ('ip_allow_list', 'ip_deny_list'):
        if params.get(param) is None:
            continue
        wanted = join_ip_list(params[param])
        if (current.get(param) or '') != wanted:
            changes[param] = wanted
    ...
    if module.check_mode:
        return True, key_obj
    key_obj = client.api_keys.update(current['$key'], **changes)
    return True, key_obj
