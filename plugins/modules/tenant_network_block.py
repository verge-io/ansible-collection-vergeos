...
options:
  ...
  cidr:
    description:
      - Network block in CIDR notation (for example C(192.168.100.0/24)),
...
def get_tenant(module, client):
    try:
        return client.tenants.get(name=module.params['tenant'])
...
def resolve_network_key(module, client, name):
    try:
        return dict(client.networks.get(name=name))['$key']
    except NotFoundError:
