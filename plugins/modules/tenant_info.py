...
# `state` is the only projection tenant_info wants that the tenant module
# does not; everything else comes from the shared contract so the two modules
# cannot report different things about the same tenant.
INFO_FIELDS = TENANT_FIELDS + ['status#state as state']
...
def main():
    ...
    try:
        if module.params.get('name'):
            try:
                tenants = [resolve_one(module, client.tenants,
                                       module.params['name'], 'tenant',
                                       fields=INFO_FIELDS)]
            except NotFoundError:
                tenants = []
        else:
            tenants = client.tenants.list(fields=INFO_FIELDS)
