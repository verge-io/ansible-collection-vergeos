...
The test agreed with the bug and both were wrong together.
  ...
  B2  code read 'member_name' / 'member_type'
      ...
      platform: {'member': 'users/1', 'member_display': 'operator'}
...
def row(fixture, index=0, /, **overrides):
    ...
    The parameters are positional-only so that ANY field name can be passed as
    an override, including ones that collide with them. The first parameter is
    ``fixture`` rather than ``name`` for the same reason from the other
    direction: a node row has a ``name`` field, ``row('nodes', name='node1')``
    is the obvious way to set it, and while positional-only makes that work,
    pylint reads it as a mistake -- differently on each supported version, and
    neither message can be suppressed in a way the other accepts. Renaming the
    parameter removes the argument instead of silencing it.
    """
    captured = _load(fixture)
    ...
    if not all_rows:
        raise AssertionError(
            ...
            % (fixture, captured['_call']))

    known = fields(fixture)
    ...
    if invented:
        raise AssertionError(
            ...
            % (', '.join(repr(f) for f in invented), fixture,
