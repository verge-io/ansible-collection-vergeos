...
class TestVnetRuleInfo:
    ...
    def test_reports_pending_apply(self):
        mock_client = MagicMock()
        mock_client.networks.list.return_value = [make_network([], pending=True)]
...
class TestSdkErrorsAreHandled:
    """An _info module that answers "Unexpected error" to a server refusal
    ...
    def test_duplicate_network_names_are_refused(self):
        """Reporting a different network's rules is worse than refusing: the
        answer looks right."""
        mock_client = MagicMock()
        mock_client.networks.list.return_value = [make_network([]),
                                                  make_network([])]
