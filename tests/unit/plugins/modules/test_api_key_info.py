...
class TestSdkErrorsAreHandled:
    """An _info module that turns a server error into "Unexpected error" sends
    the operator looking in the wrong place. All five SDK exceptions share one
    handler here, so all five are checked."""
    ...
    def test_sdk_errors_do_not_fall_through_to_unexpected(self, exc):
        mock_client = MagicMock()
        mock_client.api_keys.list.side_effect = exc('server said no')
        module = make_module(base_params())
...
class TestTheColumnsAreNamed:
    def test_list_asks_for_the_columns_it_reads(self):
