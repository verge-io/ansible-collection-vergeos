...
def make_network(rules=None, running=True):
    ...
    # rules.list() is now called with an explicit fields= projection.
...
class TestSdkErrorsAreHandled:
    ...
    def test_duplicate_network_names_are_refused(self):
        """The wrong vnet here means firewall rules on something that was
        never named -- and on this platform the network carrying the UI is one
        bad match away from the one you meant."""
        mock_client = MagicMock()
...
class TestTheProjectionNamesTheJoin:
    ...
    def test_the_rule_lookup_asks_for_every_column_it_compares(self):
        ...
        asked = set(network.rules.list.call_args.kwargs['fields'])
        for column in vnet_rule.COMPARISON_FIELDS:
            assert column in asked, (
                'vnet_rule compares %r but does not fetch it' % column)
