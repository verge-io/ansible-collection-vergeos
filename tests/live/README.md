...
| `verify-cloudinit.yml` | `cloud_init` and `windows_unattend`: an identical re-apply reports `changed=false`, and check mode reports a write only when one would happen (#125) |
